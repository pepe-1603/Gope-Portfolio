...
export function useModalService(): IModalService {
  const hideModal = (id: number, result?: ModalResult) => {
    ...
    if (modalToHide) {
      ...
      console.log('Hiding modal with ID:', id, 'Component name:', (modalToHide.component as any).name);
      ...
      setTimeout(() => {
        if (modalToHide._resolver) {
          ...
          modalToHide._resolver = undefined;
        }
  ...
  const forceHideModal = (id: number, action: ModalResult['action'] = 'programmatic_close', payload: any = null) => {
    const modalToHide = activeModals.value[id];
    if (modalToHide) {
  ...
  const showModal = (component: Component, props: Record<string, any> = {}, options: ModalOptions = {}): Promise<ModalResult> => {
    return new Promise((resolve) => {
      ...
      activeModals.value[id] = {
        ...
        props: {
          ...props,
          modalId: id,
          __onConfirm: (resultFromModal: any) => hideModal(id, { id, action: 'confirm', payload: resultFromModal }),
          __onCancel: (resultFromModal: any) => hideModal(id, { id, action: 'cancel', payload: resultFromModal }),
          __onClose: (resultFromModal: any) => hideModal(id, { id, action: 'close', payload: resultFromModal }),
        },
        options: { ...options },
        show: true,
        _resolver: resolve,
